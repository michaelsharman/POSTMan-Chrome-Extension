...
$(function() {
    ...
    $('#request').append('<div class="row" style="padding-top: 30px;"><div id="env-alert" class="span7"></div></div>');
    var $envAlert = $('#env-alert');

    // Set the environment visual cue based on the environment selection
    $(document).on('click', '.environment-list-item', function () {
        var id = $(this).attr('data-id');
        var selectedEnv = pm.envManager.attributes.selectedEnv.attributes;
        var cssClass = 'info';
        ...
        if (count > 0) {
            for (j = 0; j < count; j++) {
                ...
                if (valueType === "file") {
                    ...
                    // Set the parsed body data to a Learnosity data object so we can sign the
                    // security and request packets. We'll then set this to POSTMan's paramsBodyData
                    LN.request.setBodyData(key, value);
                }
            }

            LN.request.signData();
            var LNData = LN.request.getBodyData();
            for (var LNKey in LNData) {
                paramsBodyData.append(LNKey, LNData[LNKey]);
            }
